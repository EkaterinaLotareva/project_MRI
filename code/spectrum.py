...
N_points = 20
omega_1 = 2*np.pi*63*1e6
omega_2 = 2*np.pi*64*1e6
B_ampl = np.zeros(N_points)
omegas = np.linspace(omega_1, omega_2, N_points)
...
for t in range(N_points):
	...
	omega = omegas[t]  # Циклическая частота (рад/с)
	...
	obs_points_center = np.array([[0.003, 0.0, 0.0]])

	# Магнитное поле только в центре
	B_center = b_s_l(obs_points_center, I_matrix, N_seg, n, m, all_coordinates, normals)


	B_ampl[t] = (((B_center[0][0])**2 + (B_center[0][1])**2 + (B_center[0][2])**2)**0.5)
...
# Преобразуем частоты из рад/с в МГц для удобства
frequencies_MHz = np.array(omegas) / (2 * np.pi * 1e6)

# Создаем фигуру
plt.figure(figsize=(12, 8))

# Основной график
plt.plot(frequencies_MHz, np.array(B_ampl) * 1000, 'b', linewidth=2, marker='o', markersize=2)

# Настройки графика
plt.grid(True, alpha=0.3)
plt.xlabel('Частота (МГц)', fontsize=12)
plt.ylabel('|B| в центре (мТл)', fontsize=12)
plt.title('Зависимость амплитуды магнитного поля в центре от частоты', fontsize=14)

plt.text(0.02, 0.90, f'Количество стопок - {m}', 
         transform=plt.gca().transAxes,  # координаты относительно осей (0-1)
         fontsize=12,
         verticalalignment='top',
         bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
		 
plt.text(0.02, 0.82, 'Количество колец в стопке - 5', 
         transform=plt.gca().transAxes,  # координаты относительно осей (0-1)
         fontsize=12,
         verticalalignment='top',
         bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
...
plt.text(0.02, 0.98, 
         f'Всего точек: {len(frequencies_MHz)}\n'
         f'Диапазон: {frequencies_MHz[0]:.2f} - {frequencies_MHz[-1]:.2f} МГц',
         transform=plt.gca().transAxes,
         bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5),
         verticalalignment='top')

plt.tight_layout()
plt.savefig('B_vs_frequency.png', dpi=300)
plt.show()